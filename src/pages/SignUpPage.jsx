...
import supabase from '../supabase/supabase'; // Supabase 클라이언트 임포트
import { useNavigate } from 'react-router-dom'; // 페이지 이동을 위한 useNavigate 훅
import styled from 'styled-components'; // 스타일 컴포넌트를 사용하기 위한 라이브러리
...
const SignUpPage = () => {
  ...
  const validatePassword = (password) => {
    ...
    for (const validation of validations) {
      if (!validation.test) {
        return validation.message; // 유효성 검사 실패 시 해당 메시지 반환
      }
    }
  ...
  const handleChange = (e) => {
    const { id, value } = e.target; // 입력 필드의 ID와 값 추출
    setFormData((prev) => ({ ...prev, [id]: value })); // 기존 상태를 복사하고 변경된 필드 업데이트
  };
  ...
  const signUpNewUser = async (e) => {
    ...
    if (passwordValidationError) {
      setPasswordError(passwordValidationError); // 비밀번호 유효성 검사 실패 시 메시지 설정
      return;
    }
    ...
    if (error) {
      setErrorMessage('회원가입에 실패했습니다.'); // 오류 메시지 설정
    } else {
      setSuccessMessage('축하합니다. 회원가입에 성공하셨습니다🎉'); // 성공 메시지 설정
      setShowNavigateButton(true); // 성공 후 로그인 버튼 표시
    }
  };

  // "로그인 화면으로 이동" 버튼 클릭 핸들러
  const handleNavigate = () => {
    navigate('/'); // "/login" 경로로 페이지 이동
  };
